 ...
 * Program: Relational Database Management System
    ...
    3. My method:
        ...
        2) Use double linked list to store the status of database (for undo or redo).
...
typedef struct database {
    ...
    struct database *previous; // For double linked list
    struct database *next;
}DATABASE;

// For UNDO and REDO (store the status of database double linked list)
typedef struct database_status {
    struct database *head_database_status;
    struct database *current_database_status;
}DATABASE_STATUS;
...
DATABASE_STATUS *create_database_status();
DATABASE *copy_database(DATABASE*);
void status_insert(DATABASE_STATUS*, DATABASE*);
...
int main() {
    ...
    DATABASE_STATUS *database_status = create_database_status();

    // Initialize the original status of database
    status_insert(database_status, database);
    ...
    while(1) {
        ...
        if((!strcmp(input->head->word, "INSERT") || !strcmp(input->head->word, "SELECT") || !strcmp(input->head->word, "UPDATE") || !strcmp(input->head->word, "DELETE")) ||
           (!strcmp(input->head->word, "undo") && database_status->current_database_status->previous != NULL) ||
           (!strcmp(input->head->word, "redo") && database_status->current_database_status->next != NULL)) {
        ...
        } else if(!strcmp(input->head->word, "undo") && database_status->current_database_status->previous != NULL) {
            // Back to previous status of database
            database_status->current_database_status = database_status->current_database_status->previous;
            database = copy_database(database_status->current_database_status);

            // Write the information of UNDO into log.txt
            log_SQL_to_file(input, -1, NULL, NULL, " 復原:");

        } else if(!strcmp(input->head->word, "redo") && database_status->current_database_status->next != NULL) {
            // Back to previous status of database
            database_status->current_database_status = database_status->current_database_status->next;
            database = copy_database(database_status->current_database_status);
...
DATABASE_STATUS *create_database_status() {
    DATABASE_STATUS *database_status = (DATABASE_STATUS*)malloc(sizeof(DATABASE_STATUS));
    database_status->head_database_status = NULL;
    database_status->current_database_status = NULL;

    return database_status;
}

DATABASE *copy_database(DATABASE *database) {
    DATABASE *new_database = create_database();
    ...
    while(current_people != NULL) {
        ...
        database_insert_node(new_database, node_people, NULL, 1, 1);
        current_people = current_people->next;
    }
    while(current_group != NULL) {
        node_group = create_node_group(current_group->name, current_group->group_name);
        database_insert_node(new_database, NULL, node_group, 0, 1);
        current_group = current_group->next;
    }
...
void status_insert(DATABASE_STATUS *database_status, DATABASE *database) {
    // Copy database (in another memory space)
    DATABASE *new_database = copy_database(database);

    if(database_status->head_database_status == NULL) {
        database_status->head_database_status = new_database;
        database_status->current_database_status = new_database;
    } else {
        // Clear next status of current status
        DATABASE *node_database_status = database_status->current_database_status->next;
        DATABASE *temp_database_status = NULL;
        while(node_database_status != NULL) {
            temp_database_status = node_database_status;
            node_database_status = node_database_status->next;
            free(temp_database_status);
        }

        // Insert new database into the next of current status
        database_status->current_database_status->next = new_database;
        new_database->previous = database_status->current_database_status;
        database_status->current_database_status = new_database;
    }
}
